...
@click.option('--repos-dir', '-r', default=None, help='Dir with crawled repos metadata.', show_default=True)
...
def clone_repos(start_id: Optional[int], end_id: Optional[int], crawldir: str, repos_dir: str, token: str):
    """
    ...
    if os.path.exists(os.path.join(repos_dir, "languages_config.json")):
        forward_languages_config(os.path.join(repos_dir, "languages_config.json"), repos_dir)
